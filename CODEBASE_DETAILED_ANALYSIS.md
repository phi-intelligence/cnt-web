# CNT Media Platform - Detailed Codebase Analysis

**Date:** Current Analysis  
**Status:** Complete codebase understanding through documentation review and code exploration  
**Purpose:** Comprehensive technical analysis for development and maintenance
...
- **Web Frontend:** Flutter Web (39+ screens) deployed on AWS Amplify
- **Backend:** FastAPI (Python 3.11+) on AWS EC2 with 24 route files, 17 services
- **Database:** PostgreSQL (AWS RDS) with 21 tables
- **Media Storage:** AWS S3 + CloudFront CDN
- **Real-time:** LiveKit (meetings, streaming, voice agent) + WebSocket (notifications)
...
**Frontend (Web):**
- **Framework:** Flutter Web (Dart 3.0+)
- **State Management:** Provider (13 providers)
- **Routing:** GoRouter (declarative routing)
- **HTTP Client:** http package (REST API)
- **WebSocket:** socket_io_client (Socket.IO protocol)
- **Media Players:** just_audio, video_player
- **Real-time:** livekit_client (LiveKit integration)
- **Storage:** shared_preferences, flutter_secure_storage
- **UI:** Material Design, google_fonts, shimmer, glassmorphism
...
- **AWS Integration:** boto3 (S3)
- **Real-time:** Socket.IO (AsyncServer)
- **AI Services:** OpenAI SDK, Deepgram SDK
- **Authentication:** python-jose (JWT), passlib (bcrypt)

**Infrastructure:**
- **Web Hosting:** AWS Amplify
- **Backend Hosting:** AWS EC2 (52.56.78.203, eu-west-2)
- **Database:** AWS RDS PostgreSQL
- **Media Storage:** AWS S3 (cnt-web-media bucket)
- **CDN:** CloudFront (d126sja5o8ue54.cloudfront.net)
- **Real-time:** LiveKit Server (Docker container)

---

## 2. Frontend Architecture (Flutter Web)
...
│   ├── bible/                       # Bible reader (2 files)
│   ├── community/                   # Community screens (2 files)
│   ├── creation/                     # Content creation (7 files)
...
├── services/                        # API and external services (10 files)
...
The application uses **13 Provider classes** for comprehensive state management:
...
#### 2.4.1 ApiService (Main API Service)
...
**Connection Logic:**
- Uses `AppConfig.websocketUrl` from environment
- Handles port inference issues (prevents `:0` port bug)
- Non-blocking initialization (doesn't crash app on failure)

#### 2.4.4 VideoEditingService
...
#### 2.5.1 Web-Specific Screens (39 Total)
...
**Content Screens:**
- `podcasts_screen_web.dart` - Podcast library
...
#### 2.5.2 Key Screen Implementations
...
- `content_section.dart` - Flexible content section (horizontal/vertical layouts, disc design option)
- `loading_shimmer.dart` - Loading animations (skeleton screens, shimmer effects)
- `empty_state.dart` - Empty state illustrations

**Specialized Widgets:**
- **Community:** `widgets/community/` - Post cards, comment widgets, like buttons
- **Media:** `widgets/media/` - Global audio player, sliding audio player
- **Bible:** `widgets/bible/` - Bible reader section, document selectors
- **Meeting:** `widgets/meeting/` - Meeting widgets, video participant views
- **Live Stream:** `widgets/live_stream/` - Stream widgets, broadcaster/viewer interfaces
...
- Background: `#F7F5F2` (Cream), `#FCFAF8` (Card Background)
- Text: `#2D2520` (Dark Brown), `#5A4F47` (Medium Text)
...
#### 3.5.1 MediaService (`services/media_service.py`)
...
#### 3.5.2 AuthService (`services/auth_service.py`)
...
- `create_access_token()` - Generate JWT token (30-minute expiration)

#### 3.5.3 VideoEditingService (`services/video_editing_service.py`)
...
#### 3.5.4 AudioEditingService (`services/audio_editing_service.py`)
...
#### 3.5.5 QuoteImageService (`services/quote_image_service.py`)
...
#### 3.5.6 ThumbnailService (`services/thumbnail_service.py`)
...
#### 3.5.7 LiveKitService (`services/livekit_service.py`)
...
#### 3.5.8 AIService (`services/ai_service.py`)
...
**Implementation:**
- `audio_editor_screen.dart`: Lines 119-141 (blob URL upload)
...
- `audio_editor_screen.dart`: Lines 87-157 (state restoration)
...
**Implementation:**
- `api_service.dart`: `getMediaUrl()` method (lines 146-220)
...
**Implementation:**
- `auth_provider.dart`: Lines 30-52 (token expiration check)
- `api_service.dart`: Lines 60-70 (401 error handling)
...
**Document Created:** Complete codebase analysis  
