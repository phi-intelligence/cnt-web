...
class _CommunityScreenWebState extends State<CommunityScreenWeb> {
  ...
  void _scrollToPost(int postId) {
    ...
    final index = provider.posts.indexWhere((post) {
      final postIdValue = post is Map<String, dynamic> ? post['id'] : post.id;
                ...
                builder: (context, provider, child) {
                  if (provider.isLoading && provider.posts.isEmpty) {
                    return CustomScrollView(
                      slivers: [
                        // New Post button - scrollable header
                        SliverToBoxAdapter(
                          child: Padding(
                  ...
                  return RefreshIndicator(
                    onRefresh: () async {
                      ...
                      _hasScrolledToPost =
                          false; // Reset to allow scrolling again after refresh
                    },
                          ...
                          sliver: SliverGrid(
                            ...
                            delegate: SliverChildBuilderDelegate(
                              (context, index) {
                                ...
                                if (postIdInt != null &&
                                    widget.postId == postIdInt &&
                                    !_postKeys.containsKey(postIdInt)) {
                                ...
                                final postCard = InstagramPostCard(
                                  ...
                                  onBookmark: () {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      const SnackBar(
                                          content:
                                              Text('Bookmark coming soon')),
                                    );
                                  },
                                );

                                // Highlight the target post if navigated from carousel
                                final isTargetPost = postIdInt != null &&
                                    widget.postId == postIdInt;
