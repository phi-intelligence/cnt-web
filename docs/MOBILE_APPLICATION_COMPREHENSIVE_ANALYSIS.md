# CNT Media Platform - Mobile Application Comprehensive Analysis

**Date:** December 8, 2024  
**Platform:** Flutter (Dart) - iOS & Android  
**Status:** ✅ Production Ready (Pending Store Submission)  
**Version:** 1.0.0
...
11. [Events Feature](#events-feature)
12. [Platform-Specific Configuration](#platform-specific-configuration)
13. [Dependencies & Technologies](#dependencies--technologies)
14. [Production Deployment Status](#production-deployment-status)
15. [Recent Updates & Bug Fixes](#recent-updates--bug-fixes)
...
- **Backend Communication:** REST API (http package), WebSocket (socket_io_client)
- **Authentication:** JWT tokens stored in flutter_secure_storage
- **Local Storage:** SQLite (sqflite), SharedPreferences, Secure Storage
- **Media Playback:** just_audio, video_player, audioplayers
- **Real-time Communication:** LiveKit (meetings, live streams, voice agent)
- **Maps:** flutter_map (OpenStreetMap), geolocator, geocoding

### Application Entry Point
- **Main File:** `lib/main.dart`
- **Initialization:** Environment config loading → AppRouter → Authentication check → Navigation

### Environment Configuration
- **Configuration File:** `.env` (loaded via flutter_dotenv)
- **Production URLs:** Set via `.env` or `--dart-define` flags
- **Development Defaults:** Platform-specific (localhost for iOS, 10.0.2.2 for Android emulator)
- **Required Variables:**
...
│   ├── events/          # Events feature screens
│   ├── creation/        # Content creation screens
│   ├── meeting/         # Meeting/streaming screens
│   └── ...
...
└── widgets/             # Reusable UI components
    ...
    ├── shared/          # Shared widgets (PillTextField, ContentSection, etc.)
    └── ...
```
...
1. **Home** (`HomeScreenMobile`) - Main content feed
2. **Search** (`SearchScreenMobile`) - Content search & discovery
3. **Create** (`CreateScreenMobile`) - Content creation hub
4. **Community** (`CommunityScreenMobile`) - Social posts & interactions
5. **Profile** (`ProfileScreenMobile`) - User profile & settings
...
### Overview
A new feature allowing users to host and attend community events with map-based location selection.
